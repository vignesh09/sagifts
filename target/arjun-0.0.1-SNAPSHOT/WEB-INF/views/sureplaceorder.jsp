...
<body>
	...
	<div class="container">
		...
		<h3 style="color:black;font-weight:bold;">Check your detials</h3>
		
		<div class="row">
		
		<hr />
		
		<form:form action="placeorder" method="POST" modelAttribute="user">
			<div class="col-md-7">
			...
			<div class="col-md-5">
				<div class="form-group" >
	  					...
	  					<form:input path="cphone" class="form-control" id="cphone" style="width:80%;" required="required" readonly="true"/>
				</div>
				<div class="form-group" >
	  					<label for="cmail">Confirm Mail Id *</label>
	  					<form:input path="cmail" class="form-control" id="cmail" style="width:80%;" required="required" readonly="true"/>
				</div>
			  ...
			  2. Please do not refresh the page once the "Get Order" is clicked.
			  </h4>
			  <br />
			  <div style="text-align:center;width:100%;">
			  <button type="submit" class="btn btn-lg btn-success">Get Order</button></div>
			</div>
			</form:form>
			<div style="text-align:center;">
			<div class="col-xs-6">
			<form:form action="gobacktocheckout" method="GET" modelAttribute="user">
			
			<form:hidden path="name" value="${user.name}" />
			<form:hidden path="phone" value="${user.phone}" />
			<form:hidden path="altphone" value="${user.altphone}" />
			<form:hidden path="street1" value="${user.street1}" />
			<form:hidden path="street2" value="${user.street2}" />
			<form:hidden path="city" value="${user.city}" />
			<form:hidden path="state" value="${user.state}" />
			<form:hidden path="country" value="${user.country}" />
			<form:hidden path="mail" value="${user.mail}" />
			<form:hidden path="pincode" value="${user.pincode}" />
			<form:hidden path="cphone" value="${user.cphone}" />
			<form:hidden path="cmail" value="${user.cmail}" />
			<form:hidden path="prodname" value="${user.prodname}" />
			<input type="hidden" name="shop_name" value="${shop.name}">
			<br />
			<button type="submit" class="btn btn-lg btn-primary">Edit</button></form:form></div>
			<div class="col-xs-6">
			<br />
			<button type="submit" class="btn btn-lg btn-danger">Cancel</button></div>
			</div>
