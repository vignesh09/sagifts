...
<body>
	...
	<div class="container">
		<form:form action="sureeditthiscompany" method="GET" modelAttribute="cmpny">
			<div class="form-group" >
	  					<label for="arr">Number *</label>
	  					<form:hidden path="id" value="${cmpny.id}" />
	  					<form:input path="arr" class="form-control" id="arr" style="width:80%;" value="${cmpny.arr}" required="required" readonly="true"/>
			</div>
			<div class="form-group" >
	  					<label for="name">Company Name *</label>
	  					<input type="hidden" name="oldcmpny" value="${cmpny.name}" />
	  					<form:input path="name" class="form-control" id="name" value="${cmpny.name}" style="width:80%;" required="required"/>
			</div>
			<button type="submit" class="btn btn-lg btn-success">Edit</button>
			<a href="editcompany" class="btn btn-primary btn-lg">Cancel</a>
		</form:form>
		<br />
		<form:form action="deletethiscompany" method="GET" modelAttribute="cmpny">
		<form:hidden path="id" value="${cmpny.id}" />
		<form:hidden path="arr" value="${cmpny.arr}" />
		<form:hidden path="name" value="${cmpny.name}" />
